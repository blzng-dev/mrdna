        ...
        try {
            ...
            interaction.channel.permissionOverwrites.cache.forEach((overwrite) => {
                if (overwrite.type === 1 || overwrite.type === 'member') {
                    addedMembers.push(`<@${overwrite.id}>`);
                }
            });

            const ticketOwnersText = targetUser
                ? targetUser.toString()
                : (addedMembers.length > 0 ? addedMembers.join(", ") : "Unknown (No members added to channel)");

            let participantsText = Array.from(participants).map(id => `<@${id}>`).join(", ");
            if (!participantsText) participantsText = "None";

            let finalContent = `📄 **Transcript Generated**\n**Channel:** ${interaction.channel.name}\n**Ticket For:** ${ticketOwnersText}\n**Generated By:** ${interaction.user.toString()}\n**Participants:** ${participantsText}`;
